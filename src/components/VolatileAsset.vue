	...
	<div class="p-4 bg-white rounded-lg shadow-primary">
		<div class="grid grid-cols-1 gap-4 px-5">
			<div>
				...
				<ul>
					<li v-for="stock in winners" :key="stock.id" class="flex justify-between items-center py-2 border-b px-10 py-20 shadow-primary">
			...
			<div>
				<h3 class="font-bold text-red-600 mb-2">弱勢股</h3>
				<ul>
					<li v-for="stock in losers" :key="stock.id" class="flex justify-between items-center py-2 border-b px-10 py-20 shadow-primary">
						<span class="color-gray-600">{{ stock.company }}</span>
						<span class="text-red-600">{{ stock.dayChg }}%</span>
					</li>
				</ul>
			</div>
		</div>
